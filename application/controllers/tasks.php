...
class Tasks extends RF_Controller {
    ...
    public function index()
    {
		...
		$all_tasks						= $this->subtasking($this->data['tasks']);
		if (count($all_tasks) > 0) {
				$this->data['subtasks']	= $all_tasks['subtasks'];
				$this->data['tasks']	= $all_tasks['tasks'];
		}
	 ...
	 * @access private
	 */
	private function subtasking( $tasks ) {
		
		if ($this->config->item('rfk_subtasks')) {
				return $this->task_model->process_subtasks(	$tasks 	);
		}
